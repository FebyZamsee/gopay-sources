...
public class RemoteConfigRegistrar implements ComponentRegistrar {
    public List<Component<?>> getComponents() {
        ...
        return Arrays.asList(new Component[]{Component.builder(RemoteConfigComponent.class, (Class<? super T>[]) new Class[]{FirebaseRemoteConfigInterop.class}).name("fire-rc").add(Dependency.required((Class<?>) Context.class)).add(Dependency.required((Qualified<?>) qualified)).add(Dependency.required((Class<?>) FirebaseApp.class)).add(Dependency.required((Class<?>) FirebaseInstallationsApi.class)).add(Dependency.required((Class<?>) AbtComponent.class)).add(Dependency.optionalProvider(AnalyticsConnector.class)).factory(new RemoteConfigRegistrar$$ExternalSyntheticLambda0(qualified)).eagerInDefaultApp().build(), LibraryVersionComponent.create("fire-rc", "21.6.0")});
    }
