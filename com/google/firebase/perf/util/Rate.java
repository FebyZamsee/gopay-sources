...
public class Rate {
    ...
    public double getTokensPerSeconds() {
        ...
        if (i == 1) {
            d = ((double) this.numTokensPerTotalTimeUnit) / ((double) this.numTimeUnits);
            nanos = TimeUnit.SECONDS.toNanos(1);
        } else if (i == 2) {
            d = ((double) this.numTokensPerTotalTimeUnit) / ((double) this.numTimeUnits);
            nanos = TimeUnit.SECONDS.toMicros(1);
        } else if (i != 3) {
            return ((double) this.numTokensPerTotalTimeUnit) / ((double) this.timeUnit.toSeconds(this.numTimeUnits));
        } else {
            d = ((double) this.numTokensPerTotalTimeUnit) / ((double) this.numTimeUnits);
            nanos = TimeUnit.SECONDS.toMillis(1);
        }
        return d * ((double) nanos);
    }
}
