...
public class CrashlyticsReportDataCapture {
    ...
    static final String GENERATOR = String.format(Locale.US, "Crashlytics Android SDK/%s", new Object[]{"18.6.0"});
    ...
    private CrashlyticsReport.Session.Device populateSessionDeviceData() {
        ...
        return CrashlyticsReport.Session.Device.builder().setArch(deviceArchitecture).setModel(Build.MODEL).setCores(availableProcessors).setRam(calculateTotalRamInBytes).setDiskSpace(((long) statFs.getBlockCount()) * blockSize).setSimulator(isEmulator).setState(deviceState).setManufacturer(str).setModelClass(Build.PRODUCT).build();
    }
